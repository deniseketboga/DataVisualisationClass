...
```{sql connection=my_connection} 
/* [4] Best Film 1: Joined table critics = 100 and ranked by audience */
select *
from films
join imdb_scores using (film_id)
join rotten_tomatoes_scores using (film_id)
...
```{sql connection=my_connection} 
/* [4] Best Film 2: Joined table where audience > 90, critics = 100 and ranked by IMDB likes*/
select *
from films
join imdb_scores using (film_id)
join rotten_tomatoes_scores using (film_id)
...
```{r}
# [4] Calculate average scores by genre to find best genre
genre_avg_scores <- aggregate(score ~ genre, data = master_data, FUN = mean)
best_genre <- genre_avg_scores[which.max(genre_avg_scores$score), "genre"]
cat("Best Genre:", best_genre, "\n")
```
...
We find that the most popular genre in films
...
```{r}
# [4] Calculate average scores by genre with IMDB Scores
genre_avg_scores <- aggregate(score ~ genre, data = master_data, FUN = mean)
worst_genre <- genre_avg_scores[which.min(genre_avg_scores$score), "genre"]
cat("Worst Genre:", worst_genre, "\n")
```
...
```{sql connection=my_connection}
/* [5] Analyse mean and median statistics for Drama - chatgpt */ 
SELECT
    genre,
...
```{sql connection=my_connection}
/* [5] Analyse mean and median statistics for Comedy - chatgpt */ 
SELECT
    genre,
    COUNT(*) AS count_films,
    AVG(audience_score) AS avg_audience_score,
...
```{r}
# 5
genre_med_cscores <- aggregate(critic_score ~ genre, data = master_data, FUN = median)
print(genre_med_cscores)
```
...
```{r}
# 6
# Data Visualization: Genre frequencies 

# Assuming film_data is a data frame with the genre column
...
```{r}
# 6 Data Visualization: Popular Time Periods
ggplot(data = film_data, aes(x = , y = chol)) +
...
```{sql connection=my_connection}
/* Years that had the most imdb likes -- QUESTION */ 
...
genre_avg_imdb <- dbGetQuery(con, "
  SELECT CASE
          ...
          AVG(i.score) AS avg_imdb_scores
  FROM films AS f 
  JOIN imdb_scores AS i ON i.film_id = f.film_id
  WHERE f.genre IN ('Drama', 'Music', 'Romance', 'Comedy', 'Family', 'Fantasy', 'Horror', 'Mystery', 'Adventure', 'Crime', 'Action', 'Thriller', 'Biography', 'History', 'War', 'Sci-Fi', 'Sport', 'Animation', 'Musical', 'Documentary', 'Western', 'News')
  GROUP BY 1
  HAVING AVG(i.score) < 65
")
genre_avg_imdb


ggplot(genre_avg_imdb, aes(x = genre_cleaned, y = avg_imdb_scores)) +
  geom_bar(stat = "identity", fill = "#ADD8E6") +
  labs(x = "Genres", y = "Average IMDb Scores") +
  ggtitle("Genres ranking based on IMDB Scores") +
  theme_minimal() +
  theme(plot.title = element_text(size = 12, face = "bold"),
        axis.text = element_text(size = 10),
        axis.title = element_text(size = 12, face = "bold"),
